    ...
    <div className="max-w-2xl lg:max-w-5xl xl:max-w-6xl 2xl:max-w-7xl mx-auto bg-[#18181B] px-3 py-12  text-white border-x border-[#27272C] md:px-5 lg:px-8 xl:px-12">
      <div className="container-1">
        <div className="mb-2 font-h1 text-[#A1A1AA]  ">
          <h1 className="uppercase tracking-widest text-[12px]">
            Total Projects: <label className="text-2xl">{count}</label>
          </h1>
        </div>

        <h1 className="text-4xl font-h1 font-bold pb-5">
          My Recent Projects: Here are some projects I’ve worked on in the past
          few months.
        </h1>

        <p className="text-[#A1A1AA] font-p text-lg">
          Explore my recent projects with source code available on{" "}
          <Link
            target="_blank"
            to={"https://github.com/sIUzyy"}
            className="text-white"
          >
            GitHub
          </Link>{" "}
          and deployed for your interactive experience. View my list of
          <Link target="_blank" to={"/certification"} className="text-white ">
            {" "}
            certifications{" "}
          </Link>
        ...
        {projects.map((exp, index) => (
          <div key={index} className="rounded-xl  p-4 hover:bg-[#3F3F46]/25">
            ...
            <div className="pb-5 flex justify-between w-full ">
              <h1 className="text-[#EFEFF0] font-main tracking-widest text-lg ">
                {exp.title}
              </h1>
            </div>

            <div>
              <p className="text-[#A1A1AA] font-p text-base">
                {exp.description}
              </p>
            </div>
            ...
            <div className="py-4">
              <div className="flex items-center">
                ...
                <Link
                  target="_blank"
                  to={exp.src}
                  className="ml-3 text-[#EFEFF0] font-p text-base"
                >
                  {exp.title}
                </Link>
              </div>
